    ...
    { header: 'Updated', render: (p) => formatDateTime(p.updatedAt) },
    {
      ...
      render: (p) => (
        <div className="flex flex-wrap gap-2 min-w-[300px]">
          ...
          <PermissionAction size="sm" variant="outline" action="refund.initiate" disabled={!['paid', 'partially_refunded'].includes(String(p.paymentStatus))} onClick={(event) => { event.stopPropagation(); setRefundPayment(p); }}>
